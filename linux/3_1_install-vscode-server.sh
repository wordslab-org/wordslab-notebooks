...
curl -fsSL https://code-server.dev/install.sh | sh -s -- --version 4.108.1 --method standalone --prefix $VSCODE_DIR

# Create the directory inside the workspace where all code-server state will be stored
mkdir -p $VSCODE_DATA

# Install the Microsoft Python extension
$VSCODE_DIR/bin/code-server --install-extension ms-python.python@2026.0.0 --extensions-dir $VSCODE_DATA/extensions
