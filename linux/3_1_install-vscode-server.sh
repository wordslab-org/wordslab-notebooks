...
# WARNING: starting with vscode 4.106, a new github agents UI is conflicting with continue.dev extension visually, and this UI does not work with code-server out of the box
...
# => staying on version 4.105.1 (September 2025) for now
curl -fsSL https://code-server.dev/install.sh | sh -s -- --version 4.105.1 --method standalone --prefix $VSCODE_DIR
