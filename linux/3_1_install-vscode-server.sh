...
curl -fsSL https://code-server.dev/install.sh | sh -s -- --version 4.99.4 --method standalone --prefix $VSCODE_DIR
...
CONTINUE_GLOBAL_DIR=$VSCODE_DATA/.continue $VSCODE_DIR/bin/code-server --install-extension Continue.continue@1.0.9 --extensions-dir $VSCODE_DATA/extensions
